...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab):
    ...
    self.abst = self.get_abst(self.enc_input,self.dec_input)
  ...
  def get_abst(self,src,tgt):
    m = [t for t in tgt if t not in src ]
    return round(len(m) / len(tgt),4)
